 ...
 */
enum wait_result atomic_wait32(volatile uint32_t *valp, uint32_t expected, int64_t timeout);
 ...
 * @param timeout specifies a timeout in nanoseconds, timeout < 0 never expires.
 */
enum wait_result atomic_wait64(volatile uint64_t *valp, uint64_t expected, int64_t timeout);
