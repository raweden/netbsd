

#include <sys/cdefs.h>
#include <wasm/wasm_inst.h>
#include <wasm/wasm_module.h>

// The role of this source file is not to provide a actual implementation but to act as placeholders for atomic
// operations ensuring that call sites for atomic methods are not inlined. The concept is that if the call instruction
// uses the same register set as the instruction used in wasm then the call instruction can be more or less replaced
// with the instruction for the same atomic operations.

void __panic_abort(void) __WASM_IMPORT(kern, panic_abort);

/**
 * @return Returns the number of woken up agents
 */
uint32_t __noinline atomic_notify(const uint32_t *valp, uint32_t count)
{
    __panic_abort();
    return 0;
}

void __noinline wasm32_atomic_fence(void)
{
    //__panic_abort();
    // FIXME! There is no wasm instruction that translates to this currently..
}

// Memory barrier operations
void membar_acquire(void) __attribute__((alias("wasm32_atomic_fence")));
void membar_release(void) __attribute__((alias("wasm32_atomic_fence")));
void membar_producer(void) __attribute__((alias("wasm32_atomic_fence")));
void membar_consumer(void) __attribute__((alias("wasm32_atomic_fence")));
void membar_sync(void) __attribute__((alias("wasm32_atomic_fence")));

// Deprecated memory barriers
void membar_enter(void) __attribute__((alias("wasm32_atomic_fence")));
void membar_exit(void) __attribute__((alias("wasm32_atomic_fence")));

/**
 * 
 * @see https://github.com/WebAssembly/threads/blob/main/proposals/threads/Overview.md#wait
 * @param timeout specifies a timeout in nanoseconds
 */
enum wait_result __noinline atomic_wait32(volatile uint32_t *valp, uint32_t expected, int64_t timeout)
{
    __panic_abort();
    return ATOMIC_WAIT_NOT_EQUAL;
}

// 8-bit

uint8_t __noinline atomic_load8(const uint8_t *valp)
{
    return (*valp);
}

void __noinline atomic_store8(volatile uint8_t *valp, uint8_t val)
{
    *valp = val;
}

uint8_t __noinline atomic_add8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = tmp + val;
    return tmp;
}

uint8_t __noinline atomic_sub8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = tmp - val;
    return tmp;
}

uint8_t __noinline atomic_and8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = tmp & val;
    return tmp;
}

uint8_t __noinline atomic_or8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = tmp | val;
    return tmp;
}

uint8_t __noinline atomic_xor8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = tmp ^ val;
    return tmp;
}

uint8_t __noinline atomic_xchg8(volatile uint8_t *valp, uint8_t val)
{
    uint8_t tmp = *valp;
    *valp = val;
    return tmp;
}

uint8_t __noinline atomic_cmpxchg8(volatile uint8_t *valp, uint8_t expected, uint8_t replacement)
{
    uint8_t tmp = *valp;
    if (expected == tmp) {
        *valp = replacement;
    }
    return tmp;
}

// 16-bit

uint16_t __noinline atomic_load16(const uint16_t *valp)
{
    return (*valp);
}

void __noinline atomic_store16(volatile uint16_t *valp, uint16_t val)
{
    *valp = val;
}

uint16_t __noinline atomic_add16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = tmp + val;
    return tmp;
}

uint16_t __noinline atomic_sub16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = tmp - val;
    return tmp;
}

uint16_t __noinline atomic_and16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = tmp & val;
    return tmp;
}

uint16_t __noinline atomic_or16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = tmp | val;
    return tmp;
}

uint16_t __noinline atomic_xor16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = tmp ^ val;
    return tmp;
}

uint16_t __noinline atomic_xchg16(volatile uint16_t *valp, uint16_t val)
{
    uint16_t tmp = *valp;
    *valp = val;
    return tmp;
}

uint16_t __noinline atomic_cmpxchg16(volatile uint16_t *valp, uint16_t expected, uint16_t replacement)
{
    uint16_t tmp = *valp;
    if (expected == tmp) {
        *valp = replacement;
    }
    return tmp;
}

// 32-bit

uint32_t __noinline atomic_load32(const uint32_t *valp)
{
    return (*valp);
}

void __noinline atomic_store32(volatile uint32_t *valp, uint32_t val)
{
    *valp = val;
}

/**
 *
 */
uint32_t __noinline atomic_add32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = tmp + val;
    return tmp;
}

uint32_t __noinline atomic_sub32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = tmp - val;
    return tmp;
}

uint32_t __noinline atomic_and32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = tmp & val;
    return tmp;
}

uint32_t __noinline atomic_or32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = tmp | val;
    return tmp;
}

uint32_t __noinline atomic_xor32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = tmp ^ val;
    return tmp;
}

uint32_t __noinline atomic_xchg32(volatile uint32_t *valp, uint32_t val)
{
    uint32_t tmp = *valp;
    *valp = val;
    return tmp;
}

uint32_t __noinline atomic_cmpxchg32(volatile uint32_t *valp, uint32_t expected, uint32_t replacement)
{
    uint32_t tmp = *valp;
    if (expected == tmp) {
        *valp = replacement;
    }
    return tmp;
}


// 64-bit

enum wait_result __noinline atomic_wait64(volatile uint64_t *valp, uint64_t expected, int64_t timeout)
{
    __panic_abort();
    return ATOMIC_WAIT_NOT_EQUAL;
}

uint64_t __noinline atomic_load64(const uint64_t *valp)
{
    return (*valp);
}

void __noinline atomic_store64(volatile uint64_t *valp, uint64_t val)
{
    *valp = val;
}

uint64_t __noinline atomic_add64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = tmp + val;
    return tmp;
}

uint64_t __noinline atomic_sub64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = tmp - val;
    return tmp;
}

uint64_t __noinline atomic_and64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = tmp & val;
    return tmp;
}

uint64_t __noinline atomic_or64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = tmp | val;
    return tmp;
}

uint64_t __noinline atomic_xor64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = tmp ^ val;
    return tmp;
}

uint64_t __noinline atomic_xchg64(volatile uint64_t *valp, uint64_t val)
{
    uint64_t tmp = *valp;
    *valp = val;
    return tmp;
}

uint64_t __noinline atomic_cmpxchg64(volatile uint64_t *valp, uint64_t expected, uint64_t replacement)
{
    uint64_t tmp = *valp;
    if (expected == tmp) {
        *valp = replacement;
    }
    return tmp;
}
